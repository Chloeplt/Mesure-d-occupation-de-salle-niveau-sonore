...
void mode_normal() //mode = 1
{
  ...
  if(pir_detection == 1) {
    ...
    if (sonor_lvl_high)
    {
      ...
      spr.drawString("ALLERT Confort sonore NOK", 20, 85);
      flash_the_LED(); //si il y a trop de bruit la LED clignote
    }
    else {
      Serial.println("sonor_lvl_low");
      spr.drawString("Confort sonore OK", 20, 85);
    }
    
    //affichage taux d'occupation
    taux_occupation = random(10, 101);
    spr.drawString("Taux d'occupation :", 20, 120);
    spr.drawNumber(taux_occupation, 255, 120);
    spr.drawString("%", 290, 120);
  }
